...
void MagicCube::printFace(int faceId=-1)//每调用一次将魔方通过某种方式输出到屏幕上
{
		for (int i=0;i<6;i++)
		{
...
void MagicCube::flip(int faceId,int dir)//将第faceId个面旋转，方向为dir，dir取1或-1
{
		...
		for (int j=0;j<3;j++)
				for (int k=0;k<3;k++)
						if (dir==1)
								tmp.setFace(faceId,k*3+(2-j),face[faceId][j*3+k]);
						else
								tmp.setFace(faceId,(2-k)*3+j,face[faceId][j*3+k]);
		for (int i=0;i<12;i++)
		{
				tmp.setFace(trs[faceId][(i+3*dir+12)%12][0],trs[faceId][(i+3*dir+12)%12][1],
...
void MagicCube::rotate(int dir)
{
		...
		for (int j=0;j<3;j++)
				for (int k=0;k<3;k++)
						if (dir==1)
						{
								tmp.setFace(FACE_TOP,k*3+(2-j),face[FACE_TOP][j*3+k]);
								tmp.setFace(FACE_BOTTOM,(2-k)*3+j,face[FACE_BOTTOM][j*3+k]);
						}
						else
						{
								tmp.setFace(FACE_TOP,(2-k)*3+j,face[FACE_TOP][j*3+k]);
								tmp.setFace(FACE_BOTTOM,k*3+(2-j),face[FACE_BOTTOM][j*3+k]);
						}
		for (int i=0;i<4;i++)
				for (int j=0;j<9;j++)
						tmp.setFace((i+4-dir)%4,j,face[i][j]);
		*this = tmp;
}
...
void task1()
{
		...
		for (int i=0;i<10;i++)
		{
			...
			//	mc.solve();
				char buf[10];
				sprintf(buf,"%d",i);
				FILE *fout = fopen(("task1_"+string(buf)+".out").c_str(),"w");
				mc.printPlane(fout);
				fclose(fout);
				mc.print(("task1_"+string(buf)+".in").c_str());
		}
}
